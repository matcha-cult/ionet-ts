 ...
 * A3 任务 2 · 浏览器安全断言。
 *
 * 1) 包源码（含测试自身）零 Node 内置引用（协议验收 grep 零命中——本文件刻意不出现被查字面量）；
 * 2) 源码（非测试）零裸第三方/Node 模块导入 —— 本包零 Node 依赖，零 npm 依赖；
 * 3) 构建产物（dist 全部 .js）同样零 Node 内置、纯 ESM（无 CommonJS require/process/Buffer 调用）——
...
// vite raw 读取全部 src 源码与全部 dist 产物（含本文件自检；本文件不出现被查字面量）
const sourceFiles = import.meta.glob('./**/*.ts', { as: 'raw', eager: true });
const distFiles = import.meta.glob('../dist/*.js', { as: 'raw', eager: true });

// 动态拼出 'node' + ':'，避免本文件自身出现该字面量（自检 + 验收 grep 双重约束）
const NODE_PREFIX = ['node', ':'].join('');
...
describe('浏览器安全 · 构建产物（dist，需先 build）', () => {
  const distEntries = Object.entries(distFiles);

  it('dist 产物已生成（先执行 pnpm --filter @nbb-ionet/client-protocol run build）', () => {
    expect(distEntries.length).toBeGreaterThan(0);
    expect(distEntries.some(([p]) => p.endsWith('/dist/index.js'))).toBe(true);
    expect(distEntries.some(([p]) => p.endsWith('/dist/testing.js'))).toBe(true);
  });

  it('全部产物零 Node 内置引用', () => {
    for (const [path, source] of distEntries) {
      expect(source, path).not.toMatch(NODE_SPECIFIER);
    }
  });

  it('全部产物为纯 ESM：无 CommonJS require(/process./Buffer 调用', () => {
    for (const [, source] of distEntries) {
      expect(source).not.toMatch(CJS_REQUIRE);
      expect(source).not.toMatch(PROCESS_REF);
      expect(source).not.toMatch(BUFFER_REF);
    }
  });

  it('入口产物可在 ESM 环境动态 import 且导出齐全（浏览器 ESM 可加载性）', async () => {
